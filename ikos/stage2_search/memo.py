...
class MemoManager:
    ...
    def review_batch(self, core_topic: str, overall_task: str) -> dict[str, Any]:
        """评审当前批次的备忘录。
        ...
        try:
            ...
            # 过滤掉不相关的条目
            relevant_items = [
                item for item in result.get("reviews", []) if item.get("is_relevant", False)
            ]
    ...
    def make_final_decision(
        self, all_reviews: list[dict[str, Any]], current_batch: dict[str, Any]
    ) -> dict[str, Any]:
        """做出最终决策。
    ...
    def _empty_review_result(self) -> dict[str, Any]:
        ...
        Returns:
            dict: 空结果
        """
        return {"reviews": [], "has_valuable_items": False, "suggested_new_queries": []}
