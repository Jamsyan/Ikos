...
class DataMerger:
    ...
    def merge(self, filtered_data: list[dict[str, Any]], core_topic: str) -> dict[str, Any]:
        """合并过滤后的数据。
    ...
    def _deduplicate(self, data: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for item in data:
            ...
            if url and url not in seen_urls:
                ...
                if not any(
                    content in existing.get("cleaned_content", "") for existing in unique_data
                ):
                    unique_data.append(item)
    ...
    def _confirm_relevance(
        self, data: list[dict[str, Any]], core_topic: str
    ) -> list[dict[str, Any]]:
        """确认相关性。
    ...
    def _select_information(self, data: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """信息筛选。
        ...
        # 按质量和可信度排序
        sorted_data = sorted(
            data, key=lambda x: (x.get("reliability", 0), x.get("word_count", 0)), reverse=True
        )

        # 保留前 20 个高质量数据
        return sorted_data[:20]

    def _build_knowledge_graph(self, data: list[dict[str, Any]], core_topic: str) -> dict[str, Any]:
        """构建知识图谱。
        ...
        for item in data[:10]:  # 限制节点数量
            ...
            if title and title != core_topic:
                ...
                # 添加到核心主题的边
                edges.append(
                    {"source": "core", "target": f"node_{node_id}", "relation": "related_to"}
                )
