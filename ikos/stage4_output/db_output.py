...
class DatabaseOutputter:
    ...
    def export_to_vector_db(self, structured_data: dict[str, Any]) -> dict[str, Any]:
        """导出到向量数据库。
        ...
        record_count = (
            len(structured_data.get("items", [])) if isinstance(structured_data, dict) else 0
        )
    ...
    def export_to_graph_db(self, knowledge_graph: dict[str, Any]) -> dict[str, Any]:
        """导出到图数据库。
    ...
    def export_to_relational_db(self, structured_data: dict[str, Any]) -> dict[str, Any]:
        """导出到关系数据库。
