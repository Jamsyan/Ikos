...
class OutputDispatcher:
    ...
    def __init__(self, model_provider: ModelProvider, config_path: str | None = None):
        ...
        if config_path is None:
            config_path = (
                Path(__file__).parent.parent.parent / "config" / "prompts" / "stage4_output.yaml"
            )
    ...
    def dispatch(
        ...
        results = {"files": [], "database_records": 0, "status": "success"}

        # 文件输出
        if output_type in ["file", "both"]:
            file_results = self._dispatch_files(
                structured_data, knowledge_graph, rewritten_content, formats
            )
        ...
        if output_type in ["database", "both"]:
            ...
            if db_config:
                db_results = self._dispatch_database(structured_data, knowledge_graph, db_config)
    ...
    def _dispatch_files(
        ...
        # 知识图谱输出
        kg_file = outputter.export_knowledge_graph(knowledge_graph, output_dir, format="json")
        files.append(kg_file)

        # 结构化数据输出
        data_file = outputter.export_structured_data(structured_data, output_dir, format="json")
        ...
        for fmt in formats:
            if fmt == "markdown":
                doc_file = outputter.export_document(
                    rewritten_content, output_dir, format="markdown"
                )
