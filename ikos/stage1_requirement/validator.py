...
class NetworkValidator:
    ...
    def validate(self, knowledge_point: str, core_topic: str) -> dict[str, Any]:
        ...
        # 分析搜索结果
        validation_result = self._analyze_results(knowledge_point, search_results)

        # 记录验证结果
        self.validation_results.append(
            {"knowledge_point": knowledge_point, "result": validation_result}
        )

        return validation_result

    def _generate_queries(self, knowledge_point: str, core_topic: str) -> list[str]:
        """生成验证查询。
    ...
    def _analyze_results(self, knowledge_point: str, search_results: list[Any]) -> dict[str, Any]:
        """分析搜索结果。
        ...
        evidence = [
            {"title": r.title, "url": r.url, "snippet": r.snippet} for r in search_results[:3]
        ]
    ...
    def _fallback_validate(self, knowledge_point: str) -> dict[str, Any]:
        ...
        Returns:
            dict: 验证结果
        """
        return {"verified": False, "confidence": 0.0, "evidence": [], "reason": "验证过程失败"}
